...
void CAN_GPIO_Config(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	//使用PA8 9引脚的第二功能
	GPIO_PinRemapConfig (GPIO_Remap1_CAN1 ,ENABLE);//重定向GPIO_Remap1_CAN1===0x01,  0x02  Remap2;
...
void CAN_Mode_Config(void)	
{
	...
	CAN_InitTypeStruct.CAN_Mode = CAN_Mode_LoopBack;//CAN_Mode_LoopBack CAN_Mode_Normal;//调试时建议使用回环模式，调试完再改成NORMAL
	CAN_InitTypeStruct.CAN_NART = ENABLE; //错误重传
	CAN_InitTypeStruct.CAN_RFLM = ENABLE; //不覆盖久的数据
	CAN_InitTypeStruct.CAN_TTCM = DISABLE;//时间搓
...
void CAN_Filter_Config(void)
{
	...
	CAN_FilterInitTypeStruct.CAN_FilterFIFOAssignment = CAN_Filter_FIFO0;//要发送的数据给FIFO0
	CAN_FilterInitTypeStruct.CAN_FilterNumber = 0;
	CAN_FilterInitTypeStruct.CAN_FilterScale = CAN_FilterScale_32bit;
	CAN_FilterInitTypeStruct.CAN_FilterMode = CAN_FilterMode_IdMask;//默写地方相同就可以了
															//扩展的数据帧ID为1314
