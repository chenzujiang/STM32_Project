...
void Timer_Configuration(TIM_TypeDef* TIMx)
{
	...
	TIM_OCInitTypeDef  TIM_OCInitStructure;// tim.h中的80-109
  ...
  TIM_TimeBaseInitStructure.TIM_Prescaler = 72;//分频,此处为72M，72分频
	...
	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;//TIM_OutputState_Disable则没有PWM输出
	TIM_OCInitStructure.TIM_Pulse = 5;                           //脉冲值，PWM占空比
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low;     //输出极性（在计数寄存器小于输出比较值时IO口的状态）

	
	TIM_OC3Init(TIM3,&TIM_OCInitStructure);  //用于将以上配置写入（初始化）到寄存器中
}
